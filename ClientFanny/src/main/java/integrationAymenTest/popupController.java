...
public class popupController implements Initializable {
	...
	void DisableUsersAccount(ActionEvent event) throws AddressException, MessagingException {
		if (disableButton.getText().equals("Block this user")) {
			UserManagmentDelegate.blockUser(userChoosen);
			Runnable mailing = new Runnable() {

				@Override
				public void run() {
					try {
						UserManagmentDelegate.sendMail(userChoosen.getEmail(),
			...
			UserManagmentDelegate.enableUser(userChoosen);
			Runnable mailing = new Runnable() {
			...
			UserManagmentDelegate.unblockUser(userChoosen);
			Runnable mailing = new Runnable() {
	...
	void changeUsersPicture(ActionEvent event) throws IOException {
		...
		if (confirm) {
			...
			UserManagmentDelegate.updateUser(userChoosen);
		...
		if (confirm) {
			...
			stage.setOnCloseRequest(e -> {
				if (isChanged()) {
					...
					AdminController controller = loader.getController();
					controller.setTable(UserManagmentDelegate.getAllUsers());
