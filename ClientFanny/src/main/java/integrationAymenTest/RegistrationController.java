...
public class RegistrationController implements Initializable {
			...
			public void handle(ActionEvent event) {
				...
				GalleryFinish.setOnAction(new EventHandler<ActionEvent>() {
					...
					public void handle(ActionEvent event) {
						...
						UserManagmentDelegate.addUser(gallery);
						try {
							FinalAlert();
						} catch (IOException e) {
				...
				ArtistFinish.setOnAction(new EventHandler<ActionEvent>() {
					...
					public void handle(ActionEvent event) {
						if (musicField.isSelected()) {
							Fields music = UserManagmentDelegate.findFieldsByName("Music");
							Lf.add(music);
						}
						if (PaintingsField.isSelected()) {
							Fields painting = UserManagmentDelegate.findFieldsByName("Paintings");
							Lf.add(painting);
						}
						if (photographyField.isSelected()) {
							Fields photography = UserManagmentDelegate.findFieldsByName("Photography");
							Lf.add(photography);
						}
						if (TcraftField.isSelected()) {
							Fields TC = UserManagmentDelegate.findFieldsByName("Tunisian Craft");
							Lf.add(TC);
						}
						if (scultureField.isSelected()) {
							Fields sculpture = UserManagmentDelegate.findFieldsByName("Sculpture");
							Lf.add(sculpture);
						}
						...
						UserManagmentDelegate.addUser(artist);
						for (Fields f : Lf) {
							UserManagmentDelegate.addFields(f,
									UserManagmentDelegate.findByUsername(artist.getUsername()));
						}
						try {
							FinalAlert();
						} catch (IOException e) {
	...
	public void verifyRegistrationMail(KeyEvent event) {
		Runnable mailVerification = new Runnable() {
			...
			@Override
			public void run() {
				if ((!UserManagmentDelegate.checkMailExistance(mailRegistrationTF.getText()))
						|| (!UserManagmentDelegate.verifyMail(mailRegistrationTF.getText()))) {
	...
	public void verifyRegistrationUsername(KeyEvent event) {
		Runnable usernameVerification = new Runnable() {
			...
			@Override
			public void run() {
				if (!UserManagmentDelegate.checkUsernameExistance(usernameRegistrationTF.getText())) {
